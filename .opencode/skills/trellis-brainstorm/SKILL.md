...
description: "Guides collaborative requirements discovery before implementation. Creates task directory, seeds PRD, asks high-value questions one at a time, researches technical choices, and converges on MVP scope. Use when requirements are unclear, there are multiple valid approaches, the user describes a new feature or complex task, or asks to be grilled / stress-test a plan."
---

# Brainstorm - Requirements Discovery (AI Coding Enhanced)

This skill is Trellis's Phase 1 home for `grill-me` style planning: pressure-test a plan or design before implementation, answer derivable questions by inspecting the repo first, and persist the resolved decisions to `prd.md`.
