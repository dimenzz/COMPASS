...
Load the `trellis-brainstorm` skill and iterate on prd.md with the user. Phase 1.1 uses a grill-me style loop: stress-test the plan one decision at a time, provide a recommended answer for each question, and inspect the repo/research first whenever the answer is derivable without user input.
...
| Wants to be grilled / stress-test a plan or design | `trellis-brainstorm` |
...
This step is also the correct home for `grill-me` behavior: pressure-test a plan or design before implementation, resolve decision dependencies one at a time, and keep the emerging decisions in `prd.md`.
